...
class FramePersInfo(Frame):
    ...
    BTN_NEXT = (By.CSS_SELECTOR, u"button[title='Следующая']")

    def __init__(self, settings):
        frame = (By.NAME, u'd_pers')
    ...
    def __get_current_and_max_values(self, element):
        ...
        # print u"* Clicking at 'Персонаж'"
        ints = re.findall("\d+", self.browser.get_text(element))
        assert len(ints) == 2
    ...
    def get_injury(self):
        ...
        print u"* Clicking at 'Статистика'"
        if self.browser.is_present(self.INJURY) and self.browser.is_visible(self.INJURY):
    ...
    def get_cash(self):
        ...
        self.browser.click(self.BTN_STAT)
        print u"* Clicking at 'Статистика'"
        if self.browser.is_visible(main_cash_css) and self.browser.is_visible(blue_cash_css):
    ...
    def get_player_info(self):
        ...
        astral_level = int(re.search("\d+", self.browser.get_text(self.__ASTRAL_LINE)).group())
        self.browser.close_current_window_and_focus_to_previous_one()
    ...
    def __magbook_click_spell(self, *spell_texts):
        ...
        clicked = click_spell()
        if not clicked and self.browser.is_present(self.BTN_NEXT) and self.browser.is_visible(self.BTN_NEXT):
    ...
    def __cast_spell(self, create_clone=True):
        ...
        if create_clone:
            self.__magbook_click_spell(SpellTexts.MAGBOOK_CREATE_CLONE1,
...
class FrameAction(Frame):
    ...
    SQUARE = (By.CSS_SELECTOR, u"input[value='На площадь']")
    STREET = (By.CSS_SELECTOR, u"input[value='На улицу']")
    OTHER_CASTLES = (By.CSS_SELECTOR, u"input[value='К другим замкам']")

    HOME = (By.CSS_SELECTOR, u"input[value='В Дом бойцов']")
    ROLLS = (By.CSS_SELECTOR, u"input[value='Свитки']")
    ...
    CHAOS_FIGHT = (By.CSS_SELECTOR, u"input[value='Хаотические']")
    ...
    HOME_AMULETS = (By.CSS_SELECTOR, u"input[value='Амулеты']")
    HOME_ARMORS = (By.CSS_SELECTOR, u"input[value='Латы']")
    HOME_GLOVES = (By.CSS_SELECTOR, u"input[value='Перчатки']")
    HOME_BELTS = (By.CSS_SELECTOR, u"input[value='Пояса']")
    HOME_SHIELDS = (By.CSS_SELECTOR, u"input[value='Щиты']")
    HOME_ARMS = (By.CSS_SELECTOR, u"input[value='Оружие']")
    HOME_BOOTS = (By.CSS_SELECTOR, u"input[value='Поножи']")
    HOME_RINGS = (By.CSS_SELECTOR, u"input[value='Кольца']")
    HOME_BRACES = (By.CSS_SELECTOR, u"input[value='Наручи']")

    def __init__(self, settings):
        frame = (By.NAME, u'd_act')
        Frame.__init__(self, frame, settings)
    ...
    def __put_on(self, _id):
        ...
        for i in range(items_count):
            ...
            is_element_with_id = len(item.find_elements_by_css_selector("input[value='%d']" % _id)) > 0
            if is_element_with_id:
                self.browser.click(item.find_element_by_css_selector(u"input[value*='Надеть']"))
                return True
        ...
        for e in elements:
            ...
            for tr in trs:
                if clan in self.browser.get_text(tr):
                    enter_btn = self.browser.find_descendant(e, (By.CSS_SELECTOR, u"input[value='Войти']"))
    ...
    def _cure_with_roll(self, injury):
        ...
        for tr in trs:
            ...
            if title in text:
                e = tr.find_elements(By.CSS_SELECTOR, u"input[value='Использовать']")
                if len(e):
                    self.browser.click(e[0])
                break
...
class FrameFight(FrameAction):
    ...
    ATTACK_OR_BLOCK = (By.CSS_SELECTOR, u"#d_map input[value='Удар/Блок']")
    APPLY = (By.CSS_SELECTOR, u"#d_ub input[value='Применить']")
    BLOCKING_TICK = (By.XPATH, u"//div[@id='d_ub']//img[@class='bchk0' and contains(@id, 'bl')]")
    VIEW_LOG = (By.CSS_SELECTOR, u"button[title='Просмотр повтора']")

    END_FIGHT = (By.XPATH, u"//input[contains(@onclick, 'goRC')]")
    ...
    def is_fighting(self):
        self._switch_to_frame()
        return self.is_time_left_visible() and not self.browser.is_visible(self.VIEW_LOG)
...
class FrameUserList(Frame):
    def __init__(self, settings):
        frame = (By.NAME, u'd_ulist')
        Frame.__init__(self, frame, settings)


class FrameChatAction(Frame):
    def __init__(self, settings):
        frame = (By.NAME, u'd_chatact')
        Frame.__init__(self, frame, settings)


class FrameTopMenu(Frame):
    def __init__(self, settings):
        frame = (By.NAME, u'd_menu')
        Frame.__init__(self, frame, settings)
