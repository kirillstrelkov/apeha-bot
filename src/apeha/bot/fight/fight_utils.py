...
class PlayerNeighbours(object):
    ...
    TOTAL = 360
    RADIUS = RADIUS
    RADIUS_DOUBLE = RADIUS_DOUBLE
    CLOCKWISE = "clockwise"
    ANTI_CLOCKWISE = "counter_clockwise"

    CLOCKWISE_SET = [
        (cos(radians(angle)) * RADIUS, sin(radians(angle)) * RADIUS)
        for angle in range(-60, 300, 60)
    ]
    ANTICLOCKWISE_SET = [
        (cos(radians(angle)) * RADIUS, sin(radians(angle)) * RADIUS)
        for angle in range(240, -120, -60)
    ]

    CLOCKWISE_SET2 = [
        (cos(radians(angle)) * RADIUS_DOUBLE, sin(radians(angle)) * RADIUS_DOUBLE)
        for angle in range(-60, 300, 30)
    ]
    ANTICLOCKWISE_SET2 = [
        (cos(radians(angle)) * RADIUS_DOUBLE, sin(radians(angle)) * RADIUS_DOUBLE)
        for angle in range(240, -120, -30)
    ]
...
def create_application(left_text, right_text):
    ...
    max_size = int(re.search(r"/(\d+)", rlines[0]).group(1))

    cols = rlines[1].split()
    r = re.search(r"(\d+)-(\d+)", cols[0])
    ...
    if len(rlines) > 3:
        found = re.findall(r"На ход\s*(\d+):(\d+)", rlines[-1])
        if found and len(found) > 0:
            _min, sec = found[0]
        else:
            _min, sec = 3, 0
...
def get_best_application(apps, min_level, fighting_settings=None):
    ...
    filtered_apps = [
        ...
        if (
            ...
            and not a.obstacle
            and a.map.count(fighting_settings.APP_MAP_STANDARD) == 2
        )
    ]
    if not fighting_settings.UNWANTED_PLAYERS:
        apps = filtered_apps
    else:
        apps = []
        for app in filtered_apps:
            ...
            for player in app.players:
                ...
                is_good_app = (
                    is_good_app
                    and player_name not in fighting_settings.UNWANTED_PLAYERS
                )
                if not is_good_app:
                    break
    ...
    apps = sorted(apps, key=lambda x: (x.avg_rating, x.size, x.min_level, -x.max_size))
...
def get_astral_level_from(text):
    current_lvl = SpellTexts.CURRENT_ASTRAL_LVL_TEXT + r" (\d+)"
